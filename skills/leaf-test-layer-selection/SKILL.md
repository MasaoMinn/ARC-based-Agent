...
26. Return a machine-checkable coverage obligation for every explicit requirement outcome before mapping obligations to test files. Use MUST for stated behavior and SHOULD only for non-mandatory resilience checks.
27. For each requirement-owned input or state transition, consider the smallest relevant set of happy, negative, boundary, error, persistence, authorization, and accessibility risks. Do not add categories that the requirement cannot justify.
28. Every test manifest item must map to one or more obligation ids. Every declared scenario must map to an E2E test, while combinatorial boundary checks should normally stay in Unit or Integration layers.
29. Prefer oracles that observe durable or externally visible effects. A toast, local counter, mocked response, or DOM disappearance is insufficient when the requirement owns an API, session, write, or persistence path.
30. Tests must be capable of failing before implementation for the intended behavioral reason. Avoid assertions that only prove the test harness rendered, a mock was called, or a literal value equals itself.
